/**
 * replay.js - Replay and Live mode for order book metric analysis
 * 
 * Supports two modes:
 * - REPLAY: Loads historical snapshots, computes metrics client-side
 * - LIVE: Connects via WebSocket to receive real-time snapshots during recording
 * 
 * To generate snapshots (for replay):
 *   cd backend && npm run snapshots -- --session=<session> --symbol=BTC --timeframe=1m
 * 
 * For live mode, start recorder with OHLC + book streams enabled.
 */
(function () {
  "use strict";
  ...
  // Streaming state (for active recording sessions)
  let ws = null;
  let wsReconnectTimer = null;
  let currentTimeframe = "1m"; // Track selected timeframe for live aggregation
  let currentSession = null;
  let currentSymbol = null;
  let isSessionRecording = false; // Whether current session is actively recording
  let sessionRecordingStatus = new Map(); // session -> { recording, symbols }
  let receivingHistory = true; // Flag to batch history snapshots
  let countdownInterval = null; // Countdown timer interval
  let lastBarTime = 0; // Last bar's timestamp for countdown calculation
  ...
  function updateCountdown() {
    if (!countdownEl || !isSessionRecording) return;
  ...
  function updateSystemStatus() {
    // Server status - if we can load sessions, server is running
    const serverOk = sessionRecordingStatus.size > 0 || sessionSelect.options.length > 0;
    if (serverStatusDot) {
      serverStatusDot.style.background = serverOk ? "#10b981" : "#ef4444";
    }
    if (serverStatusText) {
      serverStatusText.textContent = serverOk ? "Running on :8787" : "Not responding";
      serverStatusText.style.color = serverOk ? "#10b981" : "#ef4444";
    }
    
    // Recorder status - check if any session is recording
    let recordingSession = null;
    let recordingSymbols = [];
    for (const [sessionId, status] of sessionRecordingStatus.entries()) {
      if (status && status.recording) {
        recordingSession = sessionId;
        recordingSymbols = status.symbols || [];
        break;
      }
    }
    
    if (recorderStatusDot) {
      recorderStatusDot.style.background = recordingSession ? "#10b981" : "#64748b";
    }
    if (recorderStatusText) {
      if (recordingSession) {
        recorderStatusText.textContent = `Recording: ${recordingSession.substring(0, 25)}...`;
        recorderStatusText.style.color = "#10b981";
      } else {
        recorderStatusText.textContent = "Not recording";
        recorderStatusText.style.color = "#64748b";
      }
    }
    
    // Overall status icon
    if (systemStatusIcon) {
      if (isSessionRecording && ws && ws.readyState === WebSocket.OPEN) {
  ...
  function updateWebSocketStatus(connected, info = "") {
    if (wsStatusDot) {
      wsStatusDot.style.background = connected ? "#10b981" : "#64748b";
    }
    if (wsStatusText) {
      wsStatusText.textContent = connected ? `Connected${info ? ` (${info})` : ""}` : "Disconnected";
      wsStatusText.style.color = connected ? "#10b981" : "#64748b";
    }
    updateSystemStatus();
  }
  ...
  function updateLiveStats(snapshotCount, lastTime, lastPrice) {
    if (liveStats) {
      liveStats.style.display = isSessionRecording ? "block" : "none";
    }
    if (liveStatsContent && isSessionRecording) {
    ...
    renderDebounceTimer = setTimeout(() => {
      if (pendingComputed) {
        try {
          ...
          console.error("[Replay] Chart render error:", e);
        }
  ...
  function updateLiveIndicator(connected) {
    if (liveIndicator) {
      liveIndicator.style.display = isSessionRecording ? "flex" : "none";
  ...
  function updateSessionStatusUI() {
    ...
    const status = sessionRecordingStatus.get(session);
    const isRecording = status && status.recording;
    
    if (liveBadge) {
      liveBadge.style.display = isRecording ? "inline-block" : "none";
      liveBadge.textContent = "🔴 RECORDING";
    }
  }
  ...
  async function handleLoad() {
    ...
    if (!session) {
      setStatus("Please select a session", true);
      return;
    }
    ...
    // Check if this session is actively recording
    const status = sessionRecordingStatus.get(session);
    isSessionRecording = status && status.recording;
    
    setStatus(`Loading ${symbol} ${timeframe} data...`);

    try {
      // Load 1m snapshots and aggregate to selected timeframe
      rawSnapshots = await loadSnapshots(session, symbol, timeframe);
      currentTimeframe = timeframe; // Track for live aggregation
      snapshots = aggregateSnapshots(rawSnapshots, timeframe);
      snapshotTimeSet = new Set(rawSnapshots.map(s => s.time)); // Track raw times for duplicate detection

      if (snapshots.length === 0) {
        setStatus("No data found for this session.", true);
      ...
      setStatus(`Loaded ${snapshots.length} bars. Computing metrics...`);
      const computed = computeAllMetrics();
      renderCharts(computed);
      
      // If session is actively recording, connect WebSocket for live updates
      if (isSessionRecording) {
        setStatus(`${snapshots.length} bars loaded • 🔴 Streaming live updates...`);
        receivingHistory = false; // We already have data from HTTP
        connectWebSocket();
        startCountdown();
        updateLiveIndicator(false); // Will update to true when connected
        if (liveBadge) {
          liveBadge.style.display = "inline-block";
          liveBadge.textContent = "🔴 LIVE";
        }
      } else {
        setStatus(`${snapshots.length} bars loaded • Session complete`);
        stopCountdown();
        updateLiveIndicator(false);
        if (liveBadge) {
  ...
  async function init() {
    ...
    // Periodically refresh session list to detect new recordings
    setInterval(async () => {
      await loadSessions();
      updateSystemStatus();
    ...
    if (sessionCount > 0) {
      // Find the first recording session (if any) and select it
      for (const [sessionId, status] of sessionRecordingStatus.entries()) {
        if (status && status.recording) {
      ...
      // Auto-load the selected session
      setStatus(`Loading session...`);
      handleLoad();
    } else {
      setStatus("No sessions found. Run the recorder first.", true);
    }
  }
